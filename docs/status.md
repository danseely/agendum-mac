...
## Current milestone
Item 4 (keyboard shortcuts + menu coverage) of the five-item live-slice orchestration in `docs/orchestration-plan.md` is the active checkpoint. The goal is to extend the macOS menu bar and keyboard surface beyond the existing `Sync Now` command (`Sources/AgendumMac/AgendumMacApp.swift:13-23`) so a keyboard-only user can drive the dashboard: refresh, create a manual task, and run every per-task action (mark seen / reviewed / in progress / move to backlog / mark done / remove / open in browser) from the menu and via shortcuts. Item 4 is mostly UX surface: it touches the SwiftUI `.commands { ... }` block in `AgendumMacApp.swift`, the `TaskDashboardCommands` workflow seam in `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift`, and `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift`. No backend or helper changes are expected.
...
## In progress
- Item 4 (keyboard shortcuts + menu coverage) is in the design phase on `codex/item-4-shortcuts-menus`. The design doc `docs/design/04-shortcuts-menus.md` is being authored to specify the new menu structure, the per-action keyboard shortcuts, the focus/selection seam that lets per-task menu items target the currently-selected task, the `TaskDashboardCommands` extensions, and the matching test plan. No implementation work has started; the build phase begins after the design clears review cycle 1.

## Blocked
- None at the implementation level. Packaging-decision routing is still deferred per `docs/packaging.md` but is not blocking item-4 work.

## Next
- Complete the item-4 design doc and submit it for cycle-1 review. Revise per reviewer findings until none remain.
- Once design is approved, build item 4 on `codex/item-4-shortcuts-menus`: extend the `.commands { ... }` block in `Sources/AgendumMac/AgendumMacApp.swift`, extend `TaskDashboardCommands` (and add a focus/selection seam) in `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift` if needed, extend `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift`, and run a smoke launch.
- Review the item-4 PR (blind cycle), then validate the standard project gates (`swift build`, `swift test --enable-code-coverage`, `/opt/homebrew/bin/python3 -m unittest discover -s Tests`, `/opt/homebrew/bin/python3 Scripts/python_coverage.py`, `git diff --check`, `swift run AgendumMac` smoke).
- Ship item 4 (squash merge into `feature/mac-prototype`, fast-forward local, prune branches).
- Start item 5 (notifications + dock badge for sync results) per `docs/orchestration-plan.md` §Items.
