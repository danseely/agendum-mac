...
## Current milestone
Item 5 (notifications + dock badge for sync results) of the five-item live-slice orchestration in `docs/orchestration-plan.md` is the active checkpoint and the LAST item in the orchestration. The goal is to surface sync completion via `UNUserNotificationCenter` notifications and to drive the macOS dock badge from the existing `BackendStatusModel.hasAttentionItems` count. Scope: introduce a `Notifying` seam (mirroring `URLOpening` / `Pasteboarding`) plus a `BadgeSetting` seam in `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift` so unit tests can assert notification posting and badge transitions without touching `UNUserNotificationCenter` or `NSApplication.shared.dockTile`. The App layer (`Sources/AgendumMac/AgendumMacApp.swift`) requests notification authorization from `SettingsView` and wires `.onChange(of: backendStatus.hasAttentionItems)` to the badge seam. New permissions surface: notification authorization (granted / denied / not determined). No backend or helper changes are expected.
...
## In progress
- Item 5 (notifications + dock badge for sync results) is in the design phase on `codex/item-5-notifications-badge`. The design doc `docs/design/05-notifications-badge.md` is being authored to specify the `Notifying` and `BadgeSetting` seams on `BackendStatusModel`, the SwiftUI integration points in `AgendumMacApp` and `SettingsView` for notification authorization and badge updates, and the matching workflow-target test plan (RecordingNotifier + RecordingBadgeSetter). No implementation work has started; the build phase begins after the design clears review cycle 1.

## Blocked
- None at the implementation level. Packaging-decision routing is still deferred per `docs/packaging.md` but is not blocking item-5 work; sandbox/notification-entitlement interactions get explicit deferral notes in the design doc.

## Next
- Complete the item-5 design doc and submit it for cycle-1 review. Revise per reviewer findings until none remain.
- Once design is approved, build item 5 on `codex/item-5-notifications-badge`: introduce `Notifying` + `BadgeSetting` typealiases and seam parameters on `BackendStatusModel`, fire a notification at the end of `forceSync()` (success and failure), and wire `.onChange(of: backendStatus.hasAttentionItems)` to the badge seam in `Sources/AgendumMac/AgendumMacApp.swift`. Extend `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift` with RecordingNotifier / RecordingBadgeSetter coverage. No backend or helper changes expected.
- Review the item-5 PR (blind cycle), then validate the standard project gates (`swift build`, `swift test --enable-code-coverage`, `/opt/homebrew/bin/python3 -m unittest discover -s Tests`, `/opt/homebrew/bin/python3 Scripts/python_coverage.py`, `git diff --check`, `swift run AgendumMac` smoke).
- Ship item 5 (squash merge into `feature/mac-prototype`, fast-forward local, prune branches). After item 5 ships the five-item live-slice orchestration is complete; the next milestone is whatever falls out of the still-deferred packaging-decision routing in `docs/packaging.md`.
