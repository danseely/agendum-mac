...
## Current objective
Deliver item 5 (notifications + dock badge for sync results) of the five-item live-slice orchestration in `docs/orchestration-plan.md`. Surface sync completion via `UNUserNotificationCenter` notifications and drive the macOS dock badge from the existing `BackendStatusModel.hasAttentionItems` count (`Sources/AgendumMacWorkflow/TaskWorkflowModel.swift:404-406`). Introduce a `Notifying` seam (mirroring `URLOpening` / `Pasteboarding` from items 1 and 3) and a `BadgeSetting` seam so workflow-target tests can assert notification posts and badge transitions without touching `UNUserNotificationCenter` or `NSApplication.shared.dockTile`. The App layer (`Sources/AgendumMac/AgendumMacApp.swift`) requests notification authorization from `SettingsView` and wires `.onChange(of: backendStatus.hasAttentionItems)` to the badge seam. Likely touches only `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift`, `Sources/AgendumMac/AgendumMacApp.swift`, and `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift`. The active phase is design: author `docs/design/05-notifications-badge.md`, take it through review cycle 1, revise to no findings, then build.

## Branch
On `codex/item-5-notifications-badge` (branched from `feature/mac-prototype` at `158954c`, the post-PR-#20 squash merge tip). The branch carries a planning-doc roll-forward commit and the design doc described in the current objective.

## Repo state
- HEAD: `codex/item-5-notifications-badge`, branched from `feature/mac-prototype` at `158954c`. `feature/mac-prototype` is in sync with `origin/feature/mac-prototype` at `158954c`. Working tree clean apart from untracked `.claude/`.
- Integration branch: `feature/mac-prototype`; PR #20 (item 4 — keyboard shortcuts + menu coverage) merged on 2026-05-03 (squash merge `158954c`).
- Previous checkpoint PR: `https://github.com/danseely/agendum-mac/pull/19`, merged into `feature/mac-prototype` on 2026-05-03 (squash merge `c4a6b5a`).
...
- Last validation date: 2026-05-03 (item 4 — keyboard shortcuts + menu coverage, post-merge gate): `swift build` passed; `swift test --enable-code-coverage` passed with 103 Swift tests (+21 over the post-PR-#19 baseline of 82); `/opt/homebrew/bin/python3 -m unittest discover -s Tests` passed with 61 Python tests; `/opt/homebrew/bin/python3 Scripts/python_coverage.py` passed at 92.4% (no Python changes in item 4); `git diff --check` passed; `swift run AgendumMac` smoke held open ~5s before SIGTERM.
...
## Next actions
1. Item 5 — design review (cycle 1). The design doc `docs/design/05-notifications-badge.md` has been authored on `codex/item-5-notifications-badge`; submit it for blind review per the orchestration plan and revise until no findings remain. This is the immediate next step.
2. Item 5 — build. Implement per the approved design on `codex/item-5-notifications-badge`: introduce `Notifying` and `BadgeSetting` typealiases plus seam parameters on `BackendStatusModel.init` in `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift`, post a sync-completion notification at the end of `forceSync()` (success and failure), expose a `setBadgeForAttentionCount(_:)` method that the App layer calls from `.onChange(of: backendStatus.hasAttentionItems)`, and request notification authorization from `SettingsView` in `Sources/AgendumMac/AgendumMacApp.swift`. Extend `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift` with `RecordingNotifier` and `RecordingBadgeSetter` coverage. Run the full validation gate set and open a draft PR targeting `feature/mac-prototype`.
3. Item 5 — review and ship. Run blind PR review cycle, validate, mark ready, merge with squash, fast-forward local `feature/mac-prototype`, prune branches.
4. After item 5 ships the five-item live-slice orchestration is complete; the next milestone is whatever falls out of the still-deferred packaging-decision routing in `docs/packaging.md`.

## After checkpoint
- Item 5 is the LAST item in the five-item live-slice orchestration. After it merges, prune any merged `codex/*` remote refs and the matching local branch, and fast-forward local `feature/mac-prototype`. The orchestration is then complete; there is no item 6 in `docs/orchestration-plan.md`. The next milestone is whatever the user routes from the still-deferred packaging-decision matrix in `docs/packaging.md` (distribution channel, Python helper runtime, sandbox / MAS posture, etc.). Per `docs/orchestration-plan.md` §Per-Item Phase Machine, item 5's docs roll-forward rides as a small terminal docs PR (which the user can choose to skip) rather than being absorbed into a follow-up item, since there is no follow-up item to absorb it.
