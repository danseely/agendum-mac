...
## Next actions
1. Decide whether to run a manual `swift run AgendumMac` smoke test before treating the UI slice as fully exercised.
2. Mark PR #9 ready only when requested or when the checkpoint is explicitly approved.
3. After PR #9 merges, fast-forward `feature/mac-prototype` and clean up the topic branch/worktree artifacts.
