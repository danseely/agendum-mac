...
Item 5 (notifications + dock badge for sync results) of the five-item live-slice orchestration in `docs/orchestration-plan.md` is the active checkpoint and the LAST item in the orchestration. PR #20 merged item 4 (keyboard shortcuts + menu coverage) into `feature/mac-prototype` on 2026-05-03 (squash merge `158954c`); item 5 is now in the design phase on `codex/item-5-notifications-badge`. Scope: surface sync completion via `UNUserNotificationCenter` notifications and drive the macOS dock badge from the existing `BackendStatusModel.hasAttentionItems` count, with a `Notifying` seam (mirroring `URLOpening` / `Pasteboarding`) and a `BadgeSetting` seam so workflow-target tests can assert notification posts and badge transitions without touching `UNUserNotificationCenter` or `NSApplication.shared.dockTile`. Likely touches only `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift`, `Sources/AgendumMac/AgendumMacApp.swift` (notification authorization request from `SettingsView`; `.onChange(of: backendStatus.hasAttentionItems)` on the App body wired to the badge seam), and `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift`. No backend or helper changes expected. The design doc `docs/design/05-notifications-badge.md` is the immediate deliverable; the build phase begins after design review cycle 1 reports no findings. Packaging-decision routing remains deferred per `docs/packaging.md` (notification-entitlement / sandbox interactions in particular) and is not blocking item 5.
