...
The richer sync lifecycle and error presentation checkpoint surfaces three details the contract already returns but the UI currently flattens. `BackendStatusModel` exposes a relative `lastSyncLabel` derived from `SyncStatus.lastSyncAt`, an `attentionItemCount`-aware accent on the sync row, and a structured `PresentedError` (message + optional recovery hint + optional code) that preserves `BackendErrorPayload.recovery`/`detail`/`code` instead of collapsing them into a single `String(describing:)`. The SwiftUI `BackendStatusPanel` renders the sync row with "Last synced N min ago" plus an attention indicator when `hasAttentionItems` is true, and renders global errors as a two-line message-plus-recovery caption; per-task errors in `TaskDetail` get the same two-line treatment. The wire contract, helper, and `AgendumMacCore` types are unchanged.

The next checkpoint should focus on remaining live-slice gaps such as richer task list filtering, packaging/distribution scoping, or a settings UI for auth repair, once the prototype acceptance criteria are met.
