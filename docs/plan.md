...
The per-task error surfacing checkpoint is implemented on `codex/per-task-error-surfacing`. It adds a `taskActionErrors: [TaskItem.ID: String]` map and `errorForTask(id:)` accessor on `BackendStatusModel`, routes task-action failures (mark seen/reviewed/in progress/move to backlog/done/remove) into that map instead of the global `errorMessage`, clears a task's entry on its next successful action and clears the whole map on `refresh()`/`selectWorkspace(...)`, surfaces the per-task error inline in the SwiftUI `TaskDetail` pane, and adds fake-backed workflow tests for scoping, success-clears, multi-task isolation, and refresh/select-workspace clearing. Cross-cutting failures (refresh, workspace selection, force sync, manual task creation) continue to set the global `errorMessage` shown in the status panel.

The next checkpoint should focus on remaining live-slice gaps such as richer sync lifecycle/error presentation (surfacing `lastSyncAt`, `hasAttentionItems`, and structured `BackendErrorPayload` recovery hints) or beginning Mac packaging/distribution work once the prototype acceptance criteria are met.
