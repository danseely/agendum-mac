...
The unsigned-bundle smoke checkpoint adds `Scripts/build_app_bundle.sh`, a developer-convenience builder that runs `swift build -c release` and assembles `.build/Agendum.app/` from the produced executable. The bundle layout uses `CFBundleIdentifier = com.danseely.agendum-mac`, `CFBundleName = Agendum`, `CFBundleExecutable = Agendum` (the SwiftPM product `AgendumMac` is renamed-on-copy into `Contents/MacOS/Agendum`), `CFBundlePackageType = APPL`, `LSMinimumSystemVersion = 14.0`, and a git-derived `CFBundleShortVersionString` / `CFBundleVersion` pair (`v0.1.0` start tag, `0.1.0+dev` fallback). No icon ships (deferred). Helper discovery is unchanged: the existing executable-URL walker in `BackendClientConfiguration.discoverDevelopmentRepositoryRoot` already finds `Backend/agendum_backend_helper.py` from inside `.build/Agendum.app/Contents/MacOS/`. CI gains a single bundle-smoke step. The slice is decision-free for the seven remaining packaging questions (signing, notarization, channel, Python runtime, helper layout, `gh`, `~/.agendum`).

After this slice merges, the next code-bearing packaging slice waits on the user picking from the seven still-deferred decisions in `docs/packaging.md` (especially distribution channel and Python helper runtime). Until then, live-slice gaps such as richer task list filtering or a settings UI for auth repair remain available as alternative checkpoints.
