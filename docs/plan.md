...
The packaging-matrix documentation checkpoint adds `docs/packaging.md`, a single planning artifact that enumerates the macOS distribution channels (direct DMG/zip, Mac App Store, Homebrew cask, TestFlight) and the production runtime-distribution options for the Python backend helper (sibling-checkout, vendor-agendum tarball, PyInstaller-bundled helper, system Python with pip-install, Homebrew Python dependency), captures the trade-offs against the existing `docs/decisions.md` constraints (helper-owned SQLite, `gh` discovery on Finder launch, sibling-checkout discipline, no `.xcodeproj`), and records a concrete prototype-phase recommendation. It also appends a 2026-05-02 decisions-log entry that names the recommendation and explicitly defers the channel and Python-runtime choices to user input. No Swift, helper, wire-contract, CI, or `Scripts/` changes.

Once the user picks a distribution channel and a Python helper runtime strategy from `docs/packaging.md`, the next checkpoint can scope the first code-bearing packaging slice (likely an unsigned `.app` bundle smoke under `Scripts/build_app_bundle.sh`). Until then, live-slice gaps such as richer task list filtering or a settings UI for auth repair remain available as alternative checkpoints.
