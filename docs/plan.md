...
The structured error mapping checkpoint extends `PresentedError.from(_:)` in `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift` so every `BackendClientError` case — not only `.helperError(payload)` — yields a stable `code` and a human-readable `recovery` hint, and so unknown `Error` types fall back to a deterministic `client.unknown` envelope. The win is that the existing `BackendStatusPanel` and `TaskDetail` two-line message+recovery captions (added in PR #13) finally render actionable guidance for transport, timeout, protocol-mismatch, helper-termination, malformed-response, and unexpected-response-id failures, and a stable `client.*` namespace becomes available to later route on. Wire contract, helper, `BackendClient.swift`, `BackendClientError.description`, `AgendumBackendServicing`, and SwiftUI views are unchanged; this is a workflow-only mapping change covered by new fake-backed `TaskWorkflowModelTests`.

The next checkpoint should focus on remaining live-slice gaps such as richer task list filtering, packaging/distribution scoping, or a settings UI for auth repair, once the prototype acceptance criteria are met. As the live-slice gaps shrink, packaging is becoming the gating concern: the prototype runs cleanly under SwiftPM today but still has no signed-app distribution path, no bundled-Python answer, and no Finder-launch story for `gh` discovery.
