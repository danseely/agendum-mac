...
Item 2 (task list filtering UI) of the five-item live-slice orchestration in `docs/orchestration-plan.md` is the active checkpoint. PR #17 merged item 1 (open task URL detail action) into `feature/mac-prototype` on 2026-05-03 (squash merge `c2a6d97`); item 2 is now in the design phase on `codex/item-2-task-list-filtering`. Scope: surface the existing `task.list` filter parameters (`source`, `status`, `project`, `includeSeen`, `limit`) as SwiftUI controls so users can narrow the visible task list. The backend helper already implements all five filters (`Backend/agendum_backend/helper.py::list_tasks`) and the Swift client already exposes them through `BackendClient.listTasks(source:status:project:includeSeen:limit:)`; the work is Swift-only — workflow-model state in `AgendumMacWorkflow` plus SwiftUI controls in `Sources/AgendumMac/AgendumMacApp.swift`. The design doc `docs/design/02-task-list-filtering.md` is the immediate deliverable; the build phase begins after design review cycle 1 reports no findings. Packaging-decision routing remains deferred per `docs/packaging.md` and is not blocking item 2.
