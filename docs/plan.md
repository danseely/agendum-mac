...
Item 4 (keyboard shortcuts + menu coverage) of the five-item live-slice orchestration in `docs/orchestration-plan.md` is the active checkpoint. PR #19 merged item 3 (settings / auth-repair UI) into `feature/mac-prototype` on 2026-05-03 (squash merge `c4a6b5a`); item 4 is now in the design phase on `codex/item-4-shortcuts-menus`. Scope: extend the macOS menu bar and keyboard shortcut surface beyond the existing `Sync Now` command so a keyboard-only user can drive the dashboard — refresh, create a manual task, and run every per-task action (mark seen / reviewed / in progress / move to backlog / mark done / remove / open in browser) via menu items and shortcuts. Likely touches only `Sources/AgendumMac/AgendumMacApp.swift` (the `.commands { ... }` block and `TaskDashboardCommands`), `Sources/AgendumMacWorkflow/TaskWorkflowModel.swift` (focus/selection seam if needed for per-task menu items), and `Tests/AgendumMacWorkflowTests/TaskWorkflowModelTests.swift`. No backend or helper changes expected. The design doc `docs/design/04-shortcuts-menus.md` is the immediate deliverable; the build phase begins after design review cycle 1 reports no findings. Packaging-decision routing remains deferred per `docs/packaging.md` and is not blocking item 4.
