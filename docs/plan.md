...
PR #16 merged the unsigned `.app` smoke bundle on 2026-05-03; the developer-convenience `Scripts/build_app_bundle.sh` and `Sources/AgendumMac/Info.plist.template` are in `feature/mac-prototype` at `12cf468` with bundle identity, version policy, and helper-discovery contract recorded in `docs/decisions.md`. No short-lived `codex/*` branch is currently open.

The next code-bearing packaging slice is gated on user input for the seven still-deferred decisions in `docs/packaging.md` (distribution channel, signing identity, notarization credentials, Python helper runtime, helper-process production layout, `gh` posture, `~/.agendum` path policy). The immediate planning step is to surface those decisions to the user and record their answers in `docs/decisions.md` before scoping the next slice. If the user defers the packaging routing, alternative live-slice checkpoints (richer task list filtering, settings/auth-repair UI, keyboard-shortcut coverage) remain available on a new `codex/*` branch from current `feature/mac-prototype`.
