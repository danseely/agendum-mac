...
## 2026-05-03 — A2: `os.Logger` adopted across targets

- Decision: All three Swift targets now declare a module-internal shared `Logger` in a per-target `Logging.swift`: `AgendumMacCore` → category `backend`, `AgendumMacWorkflow` → category `workflow`, `AgendumMac` → category `ui`, all under subsystem `com.danseely.agendum-mac`. Silent `try?`-on-error sites in `BackendClient.close()` and `BackendStatusModel.defaultNotifier` were converted to `do/catch` blocks that route through `logger.error("...: \(error.localizedDescription, privacy: .public)")`. The `deinit`-path `try? input.close()` was kept intentionally (no logger context) with an explanatory comment. Lifecycle `logger.notice` events were added at backend client process spawn / restart / close / timeout / helper-terminated; `BackendStatusModel.refresh` start/ok/error; `selectWorkspace` start/ok/error; `forceSync` start/state/error/timeout/auth-token-invalidation surface; per-task actions (`markSeen` / `markReviewed` / `markInProgress` / `moveToBacklog` / `markDone` / `removeTask`) start/error via a shared `performTaskAction(name:taskID:)`; `openTaskURL` start/no-URL/open-failed; `createManualTask` start/ok/error; `refreshDiagnostics` error; UI notification authorization request outcome; and dock-badge writes. No `print(` remains in `Sources/`.
...
## 2026-05-05 — B2: shadow-port status derivation to Swift before runtime dispatch

- Decision: Start B2 / issue #33 immediately after B1 merged, by adding pure Swift status-derivation functions in the current `AgendumMacCore` target plus shared Python/Swift parity fixtures. Do not make Python `gh.py` call Swift in B2; keep the v0 helper protocol and current Python runtime behavior unchanged until later backend-engine slices have a real Swift consumer.
