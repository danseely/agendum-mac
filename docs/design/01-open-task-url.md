...
2. `testOpenTaskURLInvokesOpenerWithTaskURL` — explicitly call `recordingOpener.setNextResult(true)` (for symmetry with the failure-path test #4), populate `tasks` via refresh, call `openTaskURL(id:)` for a task whose `url` is `https://example.com/issue/42`, assert the `RecordingURLOpener.opened` array equals `[URL(string: "https://example.com/issue/42")!]` and that no error is recorded.
...
- `swift build` passes.
- `swift test --enable-code-coverage` passes; expect `AgendumMacWorkflowTests` test count to grow by +9 tests (the new tests #2-#10 in §5.2; #1 folds an extra assertion into the new #5 rather than adding a test). `AgendumMacCoreTests` count is unchanged.
...
- **Opener swap (`@Environment(\.openURL)` -> `NSWorkspace.shared.open(URL)`).** This design replaces the existing `Open in Browser` button's `@Environment(\.openURL)` call (`Sources/AgendumMac/AgendumMacApp.swift` lines 319-325) with `NSWorkspace.shared.open(URL)` routed through the `URLOpening` seam; both resolve to the user's default browser in practice but are distinct AppKit paths, and we accept the delta because `NSWorkspace.shared.open(URL)` returns a `Bool` the model can act on (populating `taskActionErrors` on failure) whereas `@Environment(\.openURL)` does not surface a result.
